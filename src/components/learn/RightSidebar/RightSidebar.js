...
const RightSidebar = ({ tableOfContents, relativePath, dependedRepos }) => {
  ...
  return (
    <Container ref={ref}>
      <ConditionalWrapper condition={isDesktop} wrapper={children => <Scrollbar>{children}</Scrollbar>}>
        <>
          {!isEmptyTableOfContent && (<TableOfContent items={tableOfContents.items} />)}
          <EditOnGithubLink filePath={relativePath} dependedRepos={dependedRepos} />
          <StarGithubRepo mb={4} />
        </>
      </ConditionalWrapper>
      <SocialBox />
