...
const CollapsibleMenu = ({children, isCollapsedHeader, onClickItem, options}) => {
  ...
  return (
    <Container>
      {React.cloneElement(children, {onClick: onClickElement, isDropdownVisible: show})}
      <animated.div style={{height, opacity, transform}}>
        <OptionsContainer ref={ref}>
          <DropdownOptions onClickItem={onClickItem} isCollapsedHeader={isCollapsedHeader} options={options} />
        </OptionsContainer>
      </animated.div>
    </Container>
  )
