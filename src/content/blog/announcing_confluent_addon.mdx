...
We're assuming here that you've already got a working cluster on Confluent Cloud (and the Kafka Command Line Tools that come with the [latest Kafka distribution](https://kafka.apache.org/quickstart)), so the first step is to configure you Ockam project to use the Confluent add-on by pointing it to your bootstrap server address:
