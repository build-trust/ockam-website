...
Typical security features in a Kafka deployment involves using a combination of authentication and Transport Layer Security (TLS), among other things, to protect data moving into and out of Kafka topics. While they do provide benefits for securing data in transit, they don't provide a complete solution to securing data as it travels _through_ Kafka. The Kafka broker will be able to temporarily see the plaintext data. Encrypting communication both into and out of your Kafka broker combined with encryption of data at rest won't be sufficient projection from a data breach if the Kafka broker or the infrastructure it is running on is compromised, as the plaintext data and the keys to decrypt the data at rest is at least temporarily available in memory whenever the broker is handling it.
