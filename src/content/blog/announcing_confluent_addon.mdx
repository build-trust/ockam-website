---
title: 'Announcing the Confluent add-on for Ockam Orchestrator'
date: '2023-03-1'
...
Typical security features in a Kafka deployment involved using a combination of authentication and Transport Layer Security (TLS), among other things, to protect data moving into and out of Kafka topics. While they do provide benefits for securing data in transit, they don't provider a complete solution to securing data as it travels _through_ Kafka. The Kafka broker will be able to temporarily see the plaintext data. Encrypting communication both into and out of your Kafka broker combined with encryption of data at rest won't be sufficient projection from a data breach if the Kafka broker itself is compromised, as the plaintext data is at least temporarily available in memory whenever the broker is handling it.

The Confluent add-on for Ockam solves these addresses these problems, while providing additional risk mitigating benefits and data integrity assurances.

## Secure your data-in-motion

<video controls>
  <source src="/blog/announcing_confluent_addon/intro.mp4" type="video/mp4" />
  Your browser does not support the video tag.
</video>

Your team has decided that integrating a stream processing platform into your systems is the best way to meet both the scaling and agility needs of your business, and Apache Kafka was the obvious choice for the platform to use. You've also decided to take advantage of using an experienced managed platform in Confluent Cloud so that your team can focus on adding value rather than running infrastructure. The development is complete, the team has a working solution where producers can send data to Confluent Cloud, and consumers are able to receive it on the other side. Communication between your producers, consumers, and Confluent Cloud is encrypted and secure using Transport Layer Security (TLS) so you're about to proceed with moving things into production usage.

A security and compliance review has surfaced a problem. While _you_ trust Confluent Cloud, the increased focus on reducing supply-chain risk means that having data move through their systems unencrypted no longer meets your security requirements. How do you mitigate the risk to your business should some other vendor in your supply chain has a security breach? You need to be able to ensure that both:

1. There is no possibility for a third-party to read the data while it's in transit
2. There is no possibility for a third-party to modify the data while it's in transit
...
We're assuming here that you've already got a working cluster on Confluent Cloud, so the first step is to configure you Ockam project to use the Confluent add-on by pointing it to your bootstrap server address:
...
As the administrator of the Ockam project, you're able to control what other identities are allowed to enroll themselves into you project by issuing unique one-time use enrollment tokens. We'll start by creating one for our consumer:​

```bash
ockam project enroll --attribute role=member > consumer.token
```

... and then one for the producer:
...
Once that completes we can now expose our secure channel on `localhost`/`127.0.0.1` on port `4000`:
...
Copy the `kafka.config` file across, and use it to create a new topic that we'll use for sending messages between the producer and consumer in this demo (in this case we've called the topic `m4`)
...
Copy over the `project.json` and `producer1.token` files from the earlier section and use it to authneticate and enroll into our Ockam project:
...
And expose our secure channel on port `5000` so we can start sending messages through Confluent Cloud:
