...
```bash
ockam project authenticate consumer.token \
  --identity consumer
```
...
Copy over the `producer1.token` file from the earlier section and use it to authenticate and enroll into our Ockam project:

```bash
ockam project authenticate producer1.token \
  --identity producer1
```
