...
Identity created, we can authenticate to our project using the enrolment token SENSOR_TOKEN we saved at the start of this section:

```bash
ockam project authenticate $VEHCILE_TOKEN \
...
```bash
ockam project authenticate $USER_TOKEN \
  --identity alice
```
