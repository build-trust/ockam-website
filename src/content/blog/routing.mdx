...
category: 'Learning'
date: '2023-07-31'
description: 'How to use Ockam routing (with the Rust API) and its benefits'
author: 'Nazmul Idris'
...
Ockam is a suite of programming libraries, command line tools, and managed cloud services
to orchestrate end-to-end encryption, mutual authentication, key management, credential
management, and authorization policy enforcement &mdash; all at massive scale. Ockam's
end-to-end [secure channels](https://docs.ockam.io/reference/command/secure-channels)
guarantee authenticity, integrity, and confidentiality of all data-in-motion at the
application layer.
...
In this blog post we will explore the Ockam Rust API and see how routing works in Ockam.
...
secure applications. Security is not something that most of us think about when we are
...
1. Lack of trust in the data your app recieves from some other entity.
   - Who sent it to my app?
   - Is it actually the data they sent my app?
   - Missing authentication, data integrity.
2. Lack of trust in the data your app sends to some other entity.
   - Who do I sent the data to?
   - Will they be able to see it along w/ anyone else?

## Our journey

In this blog post we will create two examples of Ockam nodes communicating with each other
using Ockam routing and Ockam transports. We will use the Rust API to create these Ockam
nodes and setup the Ockam routing orchestration. Ockam routing and transports enable other
...
be using the Rust API in this blog post.

When a worker is started on a [node](#node), it is given one or more addresses. The node
...
<Alert status='info'>
  ...
  [this guide](https://docs.ockam.io/reference/libraries/rust) to get the Rust toolchain setup
...
forth inside the same node. This will give us a feel for building wokers and routing at a
basic level.

<Alert status='info'>
  For more information on creating nodes and workers using the Rust API, please refer to this
...
simply a list of `adresses`. Each worker gets an `address` in a node.
...
- [Run the Rust examples in the `ockam` repo](https://github.com/build-trust/ockam/tree/develop/examples/rust).
