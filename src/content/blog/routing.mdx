...
We will also need the `Forwarder` worker that we use in the middle node. Copy and paste
the following into a new source file `/src/forwarder.rs`. This file goes in the `src`
folder and not the `examples` folder.
