...
- Approximately 1 hour of time for complete setup
...
This post shows you how to set up automated, secure token management for Amazon Timestream InfluxDB using Ockam in four steps:
 ...
 - Step 1: Create a private Amazon Timestream InfluxDB instance (30 minutes)
...
To create these resources, click the button below to launch the stack in the `us-east-1` Region, and follow the instructions:
...
- Create and attach a security group to InfluxDB that allows inbound traffic from the VPC CIDR block.
- Creates an EC2 instance in one of the subnets with the InfluxDB client downloaded to use in `/opt` directory.
...
Note: If you used the cloudformation template to create your InfluxDB instance, you will also have an ec2 machine created that you can use to run the below commands on. If not, run the commands on a machine in your VPC after downloading the InfluxDB client.
...
AWS_REGION="us-east-1" #Update as necessary
...
Enrollment tickets provision Ockam nodes. We'll create two enrollment tickets:
 ...
 - One for the Ockam _Outlet_ node that runs alongside the private InfluxDB instance containing the lease manager.
 - One for the Ockam _Inlet_ node that runs on your client devices to communicate with the Ockam Outlet node.
...
### !!steps Generate enrollment ticket for Ockam Outlet node.

We need to generate an enrollment ticket to allow a new Ockam Outlet Node to
...
<CodeTour>
### !!steps Generate enrollment ticket for Ockam Inlet node.

We need to generate an enrollment ticket to allow a new Ockam Inlet Node to
...
On the configuration page choose the region that your Amazon Timestream InfluxDB instance is
...
* **InfluxDB Leased Token Permission:** JSON array of permission objects for InfluxDB leased token in the below format. Update as needed. Leave the variable `INFLUX_ORG_ID` as it will be replaced during runtime. For this example we will use below permissions that allows read and write access to all buckets.
...
```bash
ENROLLMENT_TICKET=$(cat inlet.ticket) \
...
You can validate everything working as expected by querying the bucket using influx cli to see the data being sent to InfluxDB from telegraf client. Update `REPLACE_WITH_YOUR_BUCKET_NAME` with the name of the bucket in InfluxDB

```bash
# Check CPU metrics
./influx query 'from(bucket:"REPLACE_WITH_YOUR_BUCKET_NAME")
  |> range(start: -10h)
