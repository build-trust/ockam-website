...
realise that the status-quo approaches are no longer good enough. You don't need
...
systems. Tthese old approaches expose you transitively to an uncountable number 
...
Initech won't want to have a tight coupling to their current app instances 
and the IP addresses, they'll want the flexibility to be able to scale 
their infrastructure as required without the need to constantly inform their
...
source address, and send data or instructions into the ACME Corp network. 
...
about protecting it and guaranteeing it's integrity. 
...
The use case we're talking about though is different. It's about allowing two 
...
to plug a cable from your prod environment directly into the prod environment of 
...
hoping the we get all of the detail in all of those layer right, all of the time.
...
secure requires additional assumptions: that nobody on the internet will try to
...
risk through the network(s).

## So let's solve it!

![Connection via an agent](/blog/building-secure-saas-platforms/agent.png)

We can build security as a feature into our product within minutes! 
We're going to stop pushing the problems onto 
...
names `saas.yaml` that we'll use to setup our Ockam node that will run on the
SaaS platform:
...
relay is what allows the Initect Platform to create a portal into to the
...
GitHub server. To access the Jira and database outlets it's as simple as running:

```bash
ockam tcp-inlet create --from 0.0.0.0:7000 --via acme-relay --to jira
...
`localhost` on ports `7000` and `9000` respectively.
...
(for example, just rename it to `initech-agent`! 😉). 
