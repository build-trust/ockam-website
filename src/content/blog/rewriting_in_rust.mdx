...
away all of the details of these protocols from application developers. We impagined
...
successful. In every iteration, we found that application devevelopers would need
...
over a varity of multi-hop, multi-protocol routes.
...
become less likely to ship behavoural bugs or security vulnerabilities to production.

### Memory safety and the borrow checker

Rust’s memory safety fatures can eliminate the possibility of use-after-frees, double
frees, overflows, out-of-bounds acess, data races and many other common mistakes that is known 
to cause 60-70% of high-severity vulnerabilities in large C or C++ codebases. Rust
provides this safety without the runtime performance costs of a garbage collected language
like Java or Go. 
