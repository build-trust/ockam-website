...
```shell ! tour
curl -X PROPFIND -u user:password http://localhost:8001/ -H "Depth: 1"
```
...
```shell ! tour
curl -u user:password -O http://localhost:8001/hello.txt
```
...
```shell ! tour
curl -X DELETE -u user:password http://localhost:8001/hello.txt
```
