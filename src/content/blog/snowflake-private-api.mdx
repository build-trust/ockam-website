...
Switch back to your Snowflake console or SnowSQL and we will configure Snowpark 
...
The resource `requests` definition tells Snowpark the average resource usage 
...
The `limits` definition puts a ceiling on the amount of resource that Snowpark
...
For one final confirmation of setup we can run `SHOW SERVICES` to see DNS
information for the service we just created. You'll see that the value is 
set to an internal hostname. You'll need this value in the next step when
creating the Ockam service.
...
python API container in Snowpark:
...
In the Snowflake console or SnowSQL, run these SQL commands. Replace 
`<EGRESS_ALLOW_LIST>` and `<OCKAM_ENROLLMENT_TICKET>` strings with the values
we captured earlier. 
...
any packets it receives to `api.private.api.snowflakecomputer.internal` on 
port `8001`. If you recall this is the internal-only hostname and port that our
...
to `8001` in the container, with is a TCP inlet that forwards those packet to
relay, which are then received by the TCP outlet running in Snowpark, which
forwards them to `api.private.api.snowflakecomputer.internal:8001`, which is 
the interface that our Python API in Snowpark is listening on. 
...
```bash ! tour
curl -X GET "http://localhost:8001/connector/customers/top10"
```
