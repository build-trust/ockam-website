...
### !!steps Configure the API access

In this screen, we need to select an authorization method:
...
For this tutorial, you can select `Digest`.
...
Toggle the `Grant access to your user credentials` button, enter `user` for the user name and `password` for the user password.
