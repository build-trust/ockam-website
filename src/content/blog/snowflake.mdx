...
codetour: true
category: Learning, Quickstart
date: '2024-09-15'
description: 
image: /blog/snowflake-reverse-etl/snowflake-reverse-etl-kafka.png
author: Glenn Gillen
...
What if I told you there was a way to stream 
those insights in real-time back to the your private Kafka systems, that it'll
...
Snowflake has become The Data Cloud and the place to support workloads such as data
...
choice for building flexible, scalable, and reliable streaming platform to 
...
In this post I'm going to show you, in just a few minutes, how to securely
connect Snowflake to your private Apache Kafka broker. We will:
...
![Reverse ETL from Snowflake to Kafka](/blog/snowflake-reverse-etl/snowflake-reverse-etl-kafka.png)
...
Once we've a table, adding a stream to it's a single command where
...
experience of data moving from Snowflake to Kafka. If you've an existing Kafka 
...
![Reverse ETL from Snowflake to Kafka](/blog/snowflake-reverse-etl/aws-complete.png)
...
to install the app. This will determine the cluster where the to provision the required
compute resources for the Kafka Connector (?? Is this accurate?).
...
privileges that have the application now has. There's nothing we need 
...
![Reverse ETL from Snowflake to Kafka](/blog/snowflake-reverse-etl/aws-kafka-setup.png)
...
A outlet decrypts any messages received by the node and 
...
In this example we're giving the interpolated the value from the environment
...
We've not completed the highlighted part of the diagram below, and our Kafka
...
![Setting up Ockam Node as native Snowflake app](/blog/snowflake-reverse-etl/aws-snowflake-setup.png)
...
If the topic doesn't exists the system will create it (?? is this accurate?)
...
in the Snowflake UI, so you must manually grant several other permissions to allow 
...
![Reverse ELT with Snowflake and Kafka complete](/blog/snowflake-reverse-etl/aws-complete.png)
